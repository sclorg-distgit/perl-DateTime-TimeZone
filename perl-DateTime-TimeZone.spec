...
BuildRequires:  %{?scl_prefix}perl(File::Find::Rule)
...
BuildRequires:  %{?scl_prefix}perl(Locale::Country) >= 3.11
BuildRequires:  %{?scl_prefix}perl(Parallel::ForkManager)
BuildRequires:  sed
%endif
# Runtime
...
# Optional tests
%if !%{defined perl_bootstrap}
BuildRequires:  %{?scl_prefix}perl(DateTime) >= 0.1501
%endif
...
%global __requires_exclude %{?__requires_exclude:%__requires_exclude|}^%{?scl_prefix}perl\\(Class::Singleton\\)$
...
%{?scl:scl enable %{scl} '}perl tools/parse_olson --dir ../tzdata-%{tzversion} --version %{tzversion} \
    --jobs $JOBS --clean%{?scl:'}
%endif
